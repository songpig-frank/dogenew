-- Create a function with SECURITY DEFINER to block users
CREATE OR REPLACE FUNCTION public.admin_block_user(user_id UUID, reason TEXT)
RETURNS BOOLEAN AS $$
DECLARE
  success BOOLEAN;
BEGIN
...
-- Create a function with SECURITY DEFINER to unblock users
CREATE OR REPLACE FUNCTION public.admin_unblock_user(user_id UUID)
RETURNS BOOLEAN AS $$
DECLARE
  success BOOLEAN;
BEGIN
